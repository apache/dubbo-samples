...
DUBBO_VERSION=${DUBBO_VERSION:-3.3.0-beta.1}
if [ "$CANDIDATE_VERSIONS" == "" ];then
  CANDIDATE_VERSIONS="dubbo.version:$DUBBO_VERSION;spring.version:6.1.3;spring-boot.version:3.2.2"
...
CONFIG_FILE="native-case-configuration.yml"
...
function check_test_image() {
    #check dubbo/native-sample-test image and version
    test_image="dubbo/native-sample-test:$JAVA_VER"
    echo "Checking test image [$test_image] .. "
    docker images --format 'table {{.Repository}}:{{.Tag}}\t{{.ID}}\t{{.CreatedAt}}\t{{.Size}}' | grep $test_image
    result=$?
    if [ $result != 0 ];then
      echo "Test image not found: $test_image, please run 'bash ./build-native-test-image.sh' first."
      exit 1
    fi
}
...
echo "Test logs dir: \${project.basedir}/logs"
echo "Test reports dir: \${project.basedir}/test-reports"
...
if [ "$targetTestcases" != "" ];then
  ...
  if [ "$TEST_CASE_FILE" != "" ]; then
    ...
    # find all native-case-configuration.yml
