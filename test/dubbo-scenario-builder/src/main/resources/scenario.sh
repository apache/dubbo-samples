...
function redirect_container_logs() {
  service_name=$1
  pod_name=$(kubectl get pod -l app=${service_name} -o jsonpath='{.items[0].metadata.name}' -n ${namespace_name} > /dev/null)
  if [ -z "$pod_name" ]; then
     return 1
  fi
  ...
  pod_status=$(kubectl get pod $pod_name -n ${namespace_name} -o jsonpath='{.status.phase}' > /dev/null)
...
function wait_pod_completion() {
  ...
  do
    ...
    pod_status=$(kubectl get job ${test_service_name} -o jsonpath='{.status.conditions[0].type}' -n ${namespace_name} > /dev/null)
...
for test_service_name in ${test_service_names[@]}; do
  ...
  test_pod_name=$(kubectl get pod -l app=${test_service_name} -o jsonpath='{.items[0].metadata.name}' -n ${namespace_name} > /dev/null)
...
if [ $result -eq 0 ]; then
    succeeded_count=0
    for test_service_name in "${test_service_names[@]}"; do
      test_succeeded=$(kubectl get job "$test_service_name" -o jsonpath='{.status.succeeded}' -n "$namespace_name" > /dev/null)
