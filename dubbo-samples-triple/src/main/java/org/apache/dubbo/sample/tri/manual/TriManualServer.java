...
public class TriManualServer implements Lifecycle {
    private final int port;

    public TriManualServer(int port) {
        this.port = port;
    }

    public static void main(String[] args) throws IOException {
        TriManualServer server = new TriManualServer(TriSampleConstants.SERVER_PORT);
        server.initialize();
        server.start();
        System.in.read();
    }
    ...
    public void start() throws IllegalStateException {
        ...
        service.setRef(new ManualGreeterImpl());

        DubboBootstrap bootstrap = DubboBootstrap.getInstance();
        bootstrap.application(new ApplicationConfig("tri-manual-server"))
